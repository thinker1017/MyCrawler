package com.tcl.crawler.window;
...
import com.tcl.crawler.indexer.Indexer;
import com.tcl.crawler.indexer.Hit;
import com.tcl.crawler.cluster.Cluster;
import com.tcl.crawler.indexer.Searcher;
import com.tcl.crawler.indexer.solr.SolrSearcher;
...
public class Main {
	...
	private void initialize() {
		...
		gl_panel.setHorizontalGroup(gl_panel.createParallelGroup(
				...
				Alignment.LEADING,
				gl_panel.createSequentialGroup().addComponent(tabbedPane,
						GroupLayout.DEFAULT_SIZE, 764, Short.MAX_VALUE).addGap(
						0)));
				...
				gl_panel_2.createSequentialGroup().addGroup(
						gl_panel_2.createParallelGroup(Alignment.LEADING)
								.addComponent(scrollPane_1,
										GroupLayout.DEFAULT_SIZE, 737,
										Short.MAX_VALUE).addGroup(
										gl_panel_2.createSequentialGroup()
												.addGap(337).addComponent(
														button_1)))
						.addContainerGap()));
		gl_panel_2.setVerticalGroup(gl_panel_2.createParallelGroup(
				Alignment.LEADING).addGroup(
				gl_panel_2.createSequentialGroup().addGap(22).addComponent(
						button_1).addGap(18).addComponent(scrollPane_1,
		...
		gl_panel_3.setVerticalGroup(gl_panel_3.createParallelGroup(
				Alignment.LEADING).addGroup(
				gl_panel_3.createSequentialGroup().addGap(34).addGroup(
						gl_panel_3.createParallelGroup(Alignment.BASELINE)
								.addComponent(textField_1,
										GroupLayout.PREFERRED_SIZE,
										GroupLayout.DEFAULT_SIZE,
										GroupLayout.PREFERRED_SIZE)
								.addComponent(lblNewLabel_1)).addGap(33)
						.addGroup(
								gl_panel_3.createParallelGroup(
										Alignment.TRAILING).addComponent(
										textField_2,
										GroupLayout.PREFERRED_SIZE,
										GroupLayout.DEFAULT_SIZE,
										GroupLayout.PREFERRED_SIZE)
										.addComponent(lblIndex)).addGap(37)
						.addComponent(btnNewButton).addGap(28).addComponent(
								lblNewLabel_2).addContainerGap(194,
								Short.MAX_VALUE)));
			...
			public void actionPerformed(ActionEvent e) {
				...
				Thread t = new Thread(new Runnable() {
					public void run() {
						 ...
						 * txtDtestindex .getText()); LinkedList<Hit> list =
						 * s.search(textField.getText(),
						...
						try {
							for (PagePOJO pojo : SolrSearcher.search(textField
									.getText(), "http://localhost:8888/solr")) {
								re += pojo.title + "\n";
						...
						/*
						 * if (checkBox_2.isSelected()) { List<PagePOJO> l = new
						 * LinkedList<PagePOJO>(); while (!list.isEmpty()) {
						 * l.add(list.pop().getPagePOJO()); } clusterResult =
						 * null; try { clusterResult = new Cluster().cluster(l);
						 * } catch (IOException e1) { e1.printStackTrace(); }
						 * catch (Exception e1) { e1.printStackTrace(); }
						 * Iterator<String> it =
						 * clusterResult.keySet().iterator();
						 * comboBox.removeAllItems(); while (it.hasNext()) {
	 ...
	 * putValue(NAME, "搜索"); putValue(SHORT_DESCRIPTION,
